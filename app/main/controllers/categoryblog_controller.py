...
router = APIRouter(prefix="/category_blogs", tags=["category_blogs"])


@router.post('/create',response_model=schemas.Msg,status_code=201)
async def create_category_blogs(
    *,
...
async def update_category_blogs(
     *,
    db: Session = Depends(get_db),
...
async def delete_category_blogs(
     *,
    db: Session = Depends(get_db),
...
async def soft_delete_category_blogs(
    *,
    db: Session = Depends(get_db),
    obj_in:schemas.CategoryBlogDelete
