...
class CRUDProduct(CRUDBase[models.Product, schemas.ProductCreate,schemas.ProductUpdate]):
    ...
    def delete(cls,db:Session,uuid:str):
        ...
        if not db_obj:
            raise  HTTPException(status_code=404,detail="Product not find")
        db.delete(db_obj)
        db.commit()
        ...
        def update(cls,db:Session,obj_in:schemas.Product):
                ...
                db_obj.qte=obj_in.qte if obj_in.qte else db_obj.qte
                db_obj.pa=obj_in.pa if obj_in.pa else db_obj
                db.commit()
                db.refresh(db_obj)
                return db_obj
